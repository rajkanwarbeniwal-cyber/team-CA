...
export function GoogleButton({ redirect = "/dashboard" }: { redirect?: string }) {
  ...
  async function handleGoogle() {
    ...
    if (error) {
      setLoading(false)
      // Google provider must be enabled in Supabase Auth settings.
      toast.error(error.message || "Google sign-in is not configured yet.")
    }
  }

  return (
    <Button type="button" variant="outline" className="w-full bg-transparent" onClick={handleGoogle} disabled={loading}>
      {loading ? (
